...
length(intersect(p_list_biotype$ensembl_gene_id, p_list_biotypes_EnsDb$GENEID))
```
...
for (i in 1:nrow(p_biotypes)){
  p_biotypes_counts[i,] <- length(which(p_list_biotype$GENEBIOTYPE == p_biotypes[i,]))
}
...
##
SORRY: "gene_biotype" must be changed to "GENEBIOTYPE"
##
...
```{r}
protein_coding = p_list_biotype[which(p_list_biotype$gene_biotype == "protein_coding"), 1]
```
...
```{r}
miRNA = p_list_biotype[which(p_list_biotype$gene_biotype == "miRNA"), 1]
length(miRNA)

polymorphic_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "polymorphic_pseudogene"), 1]
length(polymorphic_pseudogene)

Mt_rRNA = p_list_biotype[which(p_list_biotype$gene_biotype == "Mt_rRNA"), 1]
length(Mt_rRNA)

lncRNA = p_list_biotype[which(p_list_biotype$gene_biotype == "lncRNA"), 1]
length(lncRNA)

processed_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "processed_pseudogene"), 1]
length(miRNA)

unprocessed_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "unprocessed_pseudogene"), 1]
length(unprocessed_pseudogene)

IG_C_gene = p_list_biotype[which(p_list_biotype$gene_biotype == "IG_C_gene"), 1]
length(IG_C_gene)

IG_V_gene = p_list_biotype[which(p_list_biotype$gene_biotype == "IG_V_gene"), 1]
length(IG_V_gene)

TR_V_gene = p_list_biotype[which(p_list_biotype$gene_biotype == "TR_V_gene"), 1]
length(TR_V_gene)

IG_C_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "IG_C_pseudogene"), 1]
length(IG_C_pseudogene)

TR_C_gene = p_list_biotype[which(p_list_biotype$gene_biotype == "TR_C_gene"), 1]
length(TR_C_gene)

transcribed_unitary_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "transcribed_unitary_pseudogene"), 1]
length(transcribed_unitary_pseudogene)

transcribed_unprocessed_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "transcribed_unprocessed_pseudogene"), 1]
length(transcribed_unprocessed_pseudogene)

transcribed_processed_pseudogene = p_list_biotype[which(p_list_biotype$gene_biotype == "transcribed_processed_pseudogene"), 1]
...
length(matched_non_coding_pathway_genes) # we end up with 58 matched genes
```
...
dim(tcga_exp_cleaned_reduced) # we end up with 14927 genes
```
