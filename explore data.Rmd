...
```{r}
means <- apply(tcga_exp_cleaned, 1, mean)
vars <- apply(tcga_exp_cleaned, 1, var)
...
```{r}
ggplot(tcga_exp_var_mean, aes(mean, variance))+ 
  geom_point(fill="tomato1",color="tomato4",shape=21) +
  ggtitle("Variance and mean of every gene ") +
  xlab("mean") +
  ylab("variance")
```
...
```{r}
tcga_exp_var_mean_orderd <- order(tcga_exp_var_mean[,"variance"])
