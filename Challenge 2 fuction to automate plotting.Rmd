...
get_top10pathways_from_pvalues = function(df_p_values, length_genesets) {
  ...
  results[[3]] <- ggplot(data = top_10, aes(x = mean_pathway, y = reorder(pathway_names, mean_pathway),
                                            fill = pathway_size
                                            ))+
    geom_dotplot(position = "identity",
                 dotsize = (pathway_size/10),
                 binwidth = 0.5,
                 binaxis = "y")+
    labs(title = names(df_p_values),
         x = "mean p-value pathway",
  ...
  results[[4]] <- ggplot(data = top_10, aes(x = mean_pathway, y = reorder(pathway_names,
                                                                          mean_pathway)))+
