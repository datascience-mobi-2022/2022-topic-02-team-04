...
get_top10pathways_from_pvalues = function(df_p_values, length_genesets) {
  ...
  ordered_names <- order(-mean_patheay[ ,1])
  top_10_names <- ordered_names[1:10]
  top_10$pathway_names <- row.names(mean_patheay)[top_10_names]
  ...
  pathway_size <- order(-mean_patheay[ ,1])
...
```{r}
get_top10pathways <- function(x) {
  get_top10pathways_from_pvalues(x, len.genesets)
}

sapply(df.p.value.list, get_top10pathways)
```
