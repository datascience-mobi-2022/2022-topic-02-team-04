...
```{r}
#deparse(substitute(df.p.value.list[[1]]))

names(df.p.value.list[1][1])
```
...
```{r}
best_function = function(df_p_values, length_genesets) {
  
  names(df_p_values[12])
