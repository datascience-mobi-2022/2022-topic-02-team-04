...
go.terms(dif_500genes, .999)
```
***

Create lists for each GO term that contain the genes that were assigned to this specific GO term
...
mxgenes <- unique(cx_goterms[1]) #490 genes were matched
goxterms <- unique(cx_goterms[2])#3500 GO terms
GO_terms = cx_goterms[2]
Terms = lapply(GO_terms, Term)
T = as.data.frame(Terms)
colnames(T) <- "GO_term"
cx_goterms = cbind(cx_goterms, T) # matrix with Ensembl ID, GO ID, and GO term
```
...
library(ggplot2)
  ...
  for (i in 1:555){
    ...
    length(intersect(go_term_genes[[i]], topVar4000_names))
    # venn diagram
    x <- list(go_term_genes[[i]], topVar = topVar4000_names)
    ggvenn(x) + 
      ggtitle("Venn diagram of names go term genes and topVar4000 genes") +
