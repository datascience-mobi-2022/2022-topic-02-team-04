...
```{r}
mean_normal <- apply(PRAD_matrix[ ,1:58], 1, mean)

mean_tumor <- apply(PRAD_matrix[ ,59:116], 1, mean)
```
...
p_value_1 <- apply(PRAD_matrix, 1, function(x) wilcox.test(x[1:58], 
                                                         x[59:116], 
                                                         paired = TRUE,
                                                         exact = FALSE)$p.value)
...
p_value_2 <- data.frame(NA_col = rep(NA, 8851))

for(index in 1:nrow(PRAD_matrix)) {
  p_value_2[index, ] <- wilcox.test(PRAD_matrix[index,1:58], 
...
p_value_3 <- matrix(nrow = 8851, ncol = 116)

for(index in 1:nrow(PRAD_matrix)) {
  p_value_3[index, ] <- wilcox.test(PRAD_matrix[index,1:58], 
                                    PRAD_matrix[index,59:116],
                                    paired = TRUE,
                                    exact = FALSE)$p.value
  }
...
hist(as.matrix(p_value_2), breaks = 20)
```
