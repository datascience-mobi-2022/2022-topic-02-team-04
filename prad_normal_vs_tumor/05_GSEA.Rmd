...
duplicated <- duplicated(ranking_score_p_value)
...
```{r}
# ties.method = "random"
dat_for_ranking$rank <- rank(ranking_score_p_value, ties.method = "random")

duplicated_ranks <- duplicated(dat_for_ranking$rank)

length(duplicated_ranks[duplicated_ranks == TRUE])

library(dplyr)
ranked_genes <- dat_for_ranking %>% arrange(desc(dat_for_ranking$rank))
```
